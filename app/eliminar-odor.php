...
// Obtém o ID da OdorExperiencia a ser eliminada
$experienciaId = $_POST['experiencia_id'] ?? null;
$odorExperienciaId = $_POST['OdorExperiencia_ID'] ?? null;

// Verifica se o ID da SubstanciaExperiencia foi fornecido
if (!$odorExperienciaId || !$experienciaId) {
...
try {
    ...
    $stmt = $conn->prepare("CALL EliminarOdorExperiencia(?)");
    $stmt->bind_param("i", $odorExperienciaId);
    $stmt->execute();
    $stmt->close();
    $conn->close();

    // Redireciona de volta para a página anterior com uma mensagem
    echo "<script>alert('Odor eliminado com sucesso.'); window.location.href = '/labrats/app/substancias-odores.php?Experiencia_ID=$experienciaId';</script>";
